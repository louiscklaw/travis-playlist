...
def filterOutPreMergeMasterBranch(branch):
  return filterOutBranch(branch, '/pre-merge-master')

def filterOutPreMergeBranch(branch):
  return filterOutBranch(branch, '/pre-merge')

def filterOutUnwantedBranch(filterOutFunctions, branch):
  output_branch = branch
  for filterOutFunction in filterOutFunctions:
    output_branch = list(filter(lambda x: filterOutFunction(x), output_branch))

  return output_branch

def getGitAllRemoteBranches():
  return sendCommand('git branch -r')

def filterAutoGeneratedBranch(remote_branches):
  ...
  result = filterOutUnwantedBranch([
    filterOutHeadBranch,
    filterOutMasterBranch,
    filterOutDevelopBranch,
    filterOutGithubPagesBranch,
    filterOutTestBranch
    ] ,result)
...
def checkAndCleanBranch():
  for branch_to_test in filterAutoGeneratedBranch(getGitAllRemoteBranches()):
